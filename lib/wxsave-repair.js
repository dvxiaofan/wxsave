...
// name). Group 2 is the src quote, group 3 is the SVG-value quote — they must
// be chosen independently since the whole point is that the two values are
// wedged together.
const ORPHAN_SVG_RE =
...
for (const p of files) {
  try {
    ...
    const { html: fixed, count } = repairHtml(html);
    if (count === 0) {
      untouched++;
      continue;
    }
